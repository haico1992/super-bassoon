...
public class UIButton : UIButtonColor
{
	...
	//HuyHQ, support for tutorial
	public bool orignalClickState = true;
	...
	Color defaultTextEffectColor;
	UILabel text;
	UISprite[] childSprite;
	/// <summary>
			...
			if (isEnabled != value)
			{
				...
				if (text!=null) {
					if (value){
						...
						text.effectColor=this.defaultTextEffectColor;
					}
					else {
						text.color= this.disabledColor;
						text.effectColor=this.disabledColor;
					}
				}
				#region experiment change children color
				if (childSprite!=null&&childSprite.Length>0){
					if (value){
						for(int i=0; i< childSprite.Length;i++){
							childSprite[i].color= this.mStartingColor;
						}

					}
					else {
						for(int i=0; i< childSprite.Length;i++){
							childSprite[i].color= this.disabledColor;
						}
					}
				}
				#endregion
	...
	protected override void OnInit ()
	{
		...
		childSprite = this.GetComponentsInChildren<UISprite> ();
		if (text != null) {
			defaultTextColor = text.color;
			defaultTextEffectColor = text.effectColor;
		}
	}
	...
	{
		if (current == null && isEnabled && clickState)
		{
			current = this;
