...
public class UIScrollView : MonoBehaviour
{
	...
	public Vector2 horizontalLimit = new Vector2(float.MinValue,float.MaxValue);
	public Vector2 verticalLimit = new Vector2(float.MinValue,float.MaxValue);
	...
	public OnDragNotification onDragging;
	/// <summary>
	...
	//	Debug.Log (mScroll);
		...
		{
			if(mTrans.localPosition.x >= horizontalLimit.y)
			{
				Vector3 pos = mTrans.localPosition;
				pos.x = horizontalLimit.y-0.1f;
				mTrans.localPosition = pos;
				clipoffset.x = -pos.x;
				mPanel.clipOffset = clipoffset ;
				return false;
			}
			else if(mTrans.localPosition.x <= horizontalLimit.x)
			{
				Vector3 pos = mTrans.localPosition;
				pos.x = horizontalLimit.x+0.1f;
				mTrans.localPosition = pos;
				clipoffset.x = -pos.x;
				mPanel.clipOffset = clipoffset ;
				return false;
		...
		{
			//if move up
			if(mTrans.localPosition.y >= horizontalLimit.y)
			{
				Vector3 pos = mTrans.localPosition;
				pos.y = horizontalLimit.y-0.1f;
				mTrans.localPosition = pos;
				clipoffset.y = -pos.y;
				mPanel.clipOffset = clipoffset ;
				return false;
			}
			else if(mTrans.localPosition.y <= horizontalLimit.x)
			{
				Vector3 pos = mTrans.localPosition;
				pos.y = horizontalLimit.x+0.1f;
				mTrans.localPosition = pos;
				clipoffset.y = -pos.y;
				mPanel.clipOffset = clipoffset ;
				return false;
